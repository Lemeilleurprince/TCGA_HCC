...
HCC_counts = data.frame()#构建dataframe
fd1 = floders[1]#第一个文件；
file_name  =list.files(paste(path,'/',fd1,sep =''))#列出文件夹fd1中的全部文件名
file_list = substr(file_name[1],1,28)#对第一个文件名进行文字截图；
data = read.table(gzfile(paste(path,'/',fd1,'/',file_name[1],sep = '')))#读取解压包gz内部的数据；
names(data) <- c('ECSG_ID',file_list)#读取第一个gz文件之后，把文件名当作列名；
HCC_counts = data#赋值为HCC_counts矩阵；
for (fd in floders[2:179]){#循环对179个文件进行处理；
  files_name = list.files(paste(path,'/',fd,sep = ''))
  print(files_name[1])
  file_list =substr(files_name[1],1,28)
  data = read.table(gzfile(paste(path,'/',fd,'/',files_name[1],sep = '')))
  names(data) <- c('ECSG_ID',file_list)
  HCC_counts <- merge(HCC_counts,data,by ='ECSG_ID')#进行逐个以ECSG_ID编号进行合并；
  
}
  write.csv(HCC_counts,'./HCC/HCC_counts.csv')#把数据写入csv文件中
}
setwd("./HCC")
library(rjson)
...
exprSet<-cbind(hgnc_symbol,exprSet[1:56543,])
exprSet<-t(exprSet)
exprSet[1:4,1:4]
